...
def wider2face(root, phase='test', ignore_small=0):
    data = {}
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        ...
        #print('Run command: python3 test2yolo.py /path/to/original/widerface [/path/to/save/widerface/test]')
        exit(1)
    elif len(sys.argv) > 3:
    ...
    if not os.path.isfile(os.path.join(root_path, 'test', 'label.txt')):
    ...
    if len(sys.argv) == 2:
        if not os.path.isdir('widerface'):
            os.mkdir('widerface')
        if not os.path.isdir('widerface/test'):
            os.mkdir('widerface/test')

        save_path = 'widerface/test'
    else:
        save_path = sys.argv[2]

    datas = wider2face(root_path, phase='test')
